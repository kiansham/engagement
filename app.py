...
def create_chart(
    data: pd.Series, title: str, xlab: str, chart_type: str = "bar"
) -> go.Figure:
    ...
    if chart_type == "bar":
        fig = px.bar(
            ...
            labels={"x": "", "y": ""},
            color=data.index,
            color_discrete_sequence=Config.CB_SAFE_PALETTE,
        )
        fig.update_layout(
            **CHART_CONFIGS["bar"],
...
def create_status_chart(data: pd.DataFrame) -> go.Figure:
    ...
    for i, (label, value) in enumerate(
        [("On‑time", on_time), ("Late", late), ("Open", active)]
    ):
        fig.add_bar(
            y=["Engagements"],
            x=[value],
...
def create_outcome_status_chart(data: pd.DataFrame) -> go.Figure:
    ...
    for i, (label, value) in enumerate(outcome_counts.items()):
        ...
        fig.add_bar(
            y=["Outcomes"], x=[value], name=label, orientation="h", marker_color=color
        )
...
def create_info_display(items: List[tuple], use_html: bool = False) -> None:
    ...
    for col, (label, value) in zip(cols, items):
        if use_html:
            col.markdown(
                f"**{label}**<br><span style='font-size: 1.1em;'>{value}</span>",
...
def create_alert_section(
    urgent_tasks: pd.DataFrame, overdue_tasks: pd.DataFrame
) -> None:
    ...
    if not urgent_tasks.empty or not overdue_tasks.empty:
        ...
        if not urgent_tasks.empty:
            col1.markdown(
                f"""<div class="alert-urgent"><strong>⚠️ {len(urgent_tasks)} Urgent Tasks</strong><br>
                         Due within 3 days</div>""",
                unsafe_allow_html=True,
            )

        if not overdue_tasks.empty:
            col2.markdown(
                f"""<div class="alert-warning"><strong>📅 {len(overdue_tasks)} Overdue Tasks</strong><br>
                         Past due date</div>""",
                unsafe_allow_html=True,
            )


def company_selector_widget(
    full_df: pd.DataFrame, filtered_df: pd.DataFrame
) -> Optional[str]:
    """
    ...
    if not filtered_df.empty and len(filtered_df) < len(full_df):
        ...
        st.info(
            f"Showing {len(available_companies)} companies based on current filters. Clear filters to see all companies."
        )
    else:
...
def display_interaction_history(engagement_id: int) -> None:
    ...
    try:
        ...
        interactions_df = interactions_df.sort_values(
            by="interaction_date", ascending=False
        )
        ...
        if len(interactions_df) > 1:
            ...
            color_map = {
                itype: color
                for itype, color in zip(unique_types, Config.CB_SAFE_PALETTE)
            }

            for _, row in interactions_df.iterrows():
                fig.add_trace(
                    go.Scatter(
                        x=[row["interaction_date"]],
                        y=[1],
                        mode="markers+text",
                        marker=dict(
                            size=15,
                            color=color_map.get(row["interaction_type"], "#333"),
                        ),
                        text=row.get("interaction_type", "N/A"),
        ...
        for _, interaction in interactions_df.iterrows():
            with st.expander(
                f"{interaction.get('interaction_type', 'N/A')} - {interaction['interaction_date'].strftime('%Y-%m-%d')}"
            ):
                col1, col2 = st.columns(2)
                col1.markdown(f"**Milestone:** {interaction.get('milestone', 'N/A')}")
                col1.markdown(
                    f"**Outcome:** {interaction.get('outcome_status', 'N/A')}"
                )
                col2.markdown(f"**Logged By:** {interaction.get('logged_by', 'N/A')}")
                col2.markdown(
                    f"**Logged Date:** {interaction.get('logged_date', 'N/A')}"
                )
                st.markdown("**Summary:**")
                st.write(
                    interaction.get("interaction_summary", "No summary available.")
                )
...
def sidebar_filters(df: pd.DataFrame) -> tuple:
    with st.sidebar:
        ...
        with st.expander("Themes & Content", expanded=False):
            ...
            interaction_search = st.text_input(
                "Search Interaction Summary", help="Search within interaction summaries"
            )
    ...
    if interaction_search and "interactions" in filtered_df.columns:
        ...
        def search_interactions(interactions_json):
            try:
                ...
                return any(
                    interaction_search.lower()
                    in interaction.get("interaction_summary", "").lower()
...
def dashboard():
    ...
    if data.empty:
        st.warning(
            "No engagement data available or no data matches the current filters."
        )
        return
    ...
    create_metric_row(
        [
            ...
            ("On‑time Effectiveness", f"{effectiveness:.1f}%"),
    ...
    with context_col:
        ...
        st.markdown(
            """Shows the breakdown of engagement statuses, displaying those that are:
        \\
        • Currently active \\
        • Complete \\
        • Not yet started"""
    ...
    if "outcome_status" in data.columns and not data["outcome_status"].dropna().empty:
        ...
        with context_col:
            ...
            st.markdown(
                """Shows the distribution of interaction outcomes, including:
            \\
            • Positive responses \\
            • Neutral engagements \\
            • Negative feedback \\
    ...
    if "gics_sector" in data.columns and not data["gics_sector"].dropna().empty:
        ...
        with context_col:
            ...
            st.write(
                "Distribution of engagements across different GICS sectors, helping identify which industries are most actively engaged."
            )
        with chart_col:
            st.plotly_chart(
                create_chart(
    ...
    if "region" in data.columns and not data["region"].dropna().empty:
        ...
        with context_col:
            ...
            st.write(
                "Geographic spread of engagements by region, providing insights into global engagement coverage and regional focus areas."
            )
        with chart_col:
            st.plotly_chart(
                create_chart(
                    data["region"].value_counts(), "Engagements by Region", "Region"
                ),
                use_container_width=True,
            )
    ...
    if "milestone" in data.columns and not data["milestone"].dropna().empty:
        ...
        with context_col:
            ...
            st.write(
                "Current milestone stages across all engagements, showing progress through the engagement lifecycle from initiation to completion."
            )
        with chart_col:
            st.plotly_chart(
                create_chart(
                    data["milestone"].value_counts(),
                    "Engagements by Milestone Stage",
                    "Milestone",
                ),
                use_container_width=True,
            )
...
def engagement_operations():
    ...
    with tab1:
        with st.form("new_engagement", clear_on_submit=True):
            ...
            gics_sector = col2.selectbox(
                "GICS Sector *", [""] + get_lookup_values("gics_sector")
            )

            col1, col2, col3 = st.columns(3)
            isin = col1.text_input(
                "ISIN", help="International Securities Identification Number"
            )
            ...
            target_date = col2.date_input(
                "Target Date", value=datetime.now().date() + timedelta(days=90)
            )
    ...
    with tab2:
        ...
        with st.form("log_interaction"):
            ...
            col1, col2 = st.columns(2)
            interaction_type = col1.selectbox(
                "Interaction Type *", [""] + get_lookup_values("interaction_type")
            )
            interaction_date = col2.date_input(
                "Interaction Date *", value=datetime.now().date()
            )

            col1, col2 = st.columns(2)
            outcome_status = col1.selectbox(
                "Outcome Status *", [""] + get_lookup_values("outcome_status")
            )
            escalation_level = col2.selectbox(
                "New Escalation Level", [""] + get_lookup_values("escalation_level")
            )
            ...
            col1, col2 = st.columns(2)
            milestone = col1.selectbox(
                "New Milestone", [""] + get_lookup_values("milestone")
            )
            milestone_status = col2.selectbox(
                "New Milestone Status", [""] + get_lookup_values("milestone_status")
            )
                ...
                if not interaction_summary.strip() or not interaction_type:
                    ...
                    interaction_log_data = {
                        ...
                        "escalation_level": (
                            escalation_level
                            if escalation_level
                            else engagement_data.get("escalation_level")
                        ),
                        "milestone": (
                            milestone if milestone else engagement_data.get("milestone")
                        ),
                        "milestone_status": (
                            milestone_status
                            if milestone_status
                            else engagement_data.get("milestone_status")
                        ),
                    }
...
def task_management():
    ...
    warning_tasks = all_upcoming_tasks[
        (all_upcoming_tasks["days_to_next_action"] > 3)
        & (all_upcoming_tasks["days_to_next_action"] <= 7)
    ]
    ...
    if len(filtered_df) < len(st.session_state["FULL_DATA"]):
        st.info(
            f"Tasks filtered to show only companies matching current filter criteria."
        )
            ...
            if not tasks.empty:
                for _, task in tasks.iterrows():
                    with st.container():
                        ...
                        if col3.button(
                            "Mark Complete", key=f"task_{task['engagement_id']}"
                        ):
                            success, msg = update_milestone_status(
                                task["engagement_id"], "Complete"
                            )
                            if success:
                                st.success(msg)
                                st.rerun()
...
def enhanced_analysis():
    ...
    with tab1:
        ...
        if not esg_data.empty and esg_data.sum() > 0:
            st.plotly_chart(
                create_chart(esg_data, "ESG Focus Distribution", "Flag", "pie"),
                use_container_width=True,
            )
        else:
    ...
    with tab2:
        ...
        regions = ["Global"] + sorted(
            df.get("region", pd.Series(dtype=str)).dropna().unique().tolist()
        )
        selected_region = st.selectbox("View Region", regions, key="geo_region_select")

        region_df = (
            df
            if selected_region == "Global"
            else df[df.get("region") == selected_region]
        )

        if "country" in region_df.columns and not region_df.empty:
            country_data = region_df.groupby("country").size().reset_index(name="count")
            fig = px.choropleth(
                country_data,
                locations="country",
            ...
            st.info("No country data available for the selected region.")
    ...
    with tab4:
        ...
        if not analytics_data["success_rates"].empty:
            fig = px.bar(
                ...
                y="success_rate",
                title="",
                labels={"success_rate": "", "gics_sector": ""},
            )
            fig.update_layout(
